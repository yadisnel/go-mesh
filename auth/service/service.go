...
import (
	...
	"github.com/yadisnel/go-ms/v2/auth/token/jwt"
...
func (s *svc) Init(opts ...auth.Option) {
	...
	s.auth = pb.NewAuthService("go.ms.auth", s.options.Client)
	s.rules = pb.NewRulesService("go.ms.auth", s.options.Client)
...
func NewAuth(opts ...auth.Option) auth.Auth {
	...
	return &svc{
		auth:    pb.NewAuthService("go.ms.auth", options.Client),
		rules:   pb.NewRulesService("go.ms.auth", options.Client),
		options: options,
	}
}
