...
const defaultRepo = "github.com/ms/services"
...
func (s *Source) RuntimeName() string {
	if s.Repo == "github.com/ms/services" || s.Repo == "" {
		return s.Folder
	}
...
// ParseSource parses a `ms run/update/kill` source.
func ParseSource(source string) (*Source, error) {
	// If github is not present, we got a shorthand for `ms/services`
	if !strings.Contains(source, "github.com") {
		source = "github.com/ms/services/" + source
	}
...
var nameExtractRegexp = regexp.MustCompile(`((ms|web)\.Name\(")(.*)("\))`)
